...
| 移动光标 | 单指拖动 | 移动指针（自动同步） |
| 左键 | 单击；双击 = 双击 | 点按 / 单击 |
| 右键 | 长按 0.5s；两指轻点 | 两指轻点 / 双指点按 |
| 左键拖动 | —（暂不支持，用右键拖动替代） | 按住拖动 |
| 右键拖动 | 长按后拖动 | — |
| 滚轮 | 双指拖动 | 双指滚动 |
| 缩放画面 | 捏合 | 捏合 |
...
**分辨率说明**：分辨率留空时，App 会用「当前视口 × 屏幕像素密度」协商远程桌面大小，
横屏 + 全屏体验最佳；固定分辨率适合需要和桌面显示器一致的 1:1 场景。
...
- 左键拖动（按住左键移动）暂未实现触屏手势路径；可用右键拖动或硬件指针
